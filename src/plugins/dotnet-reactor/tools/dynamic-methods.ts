...
export const DynamicMethodsInputSchema = z.object({
  ...
  decompile: z.boolean().default(true).describe('Attempt to decompile recovered IL back to C#'),
  timeout_seconds: z
    .number()
    .min(5)
    .max(120)
    ...
    try {
      ...
      try {
        const artRef = await persistStaticAnalysisJsonArtifact(
          ...
          database,
          args.sample_id,
          'reactor_dynamic_methods',
          'reactor-dynamic-methods',
          result
        )
