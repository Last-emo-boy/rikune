...
const codeAnalysisPlugin: Plugin = {
  ...
  register(server, deps) {
    ...
    server.registerTool(
      codeFunctionsSmartRecoverToolDefinition,
      createCodeFunctionsSmartRecoverHandler(wm, db, cm)
    )
    ...
    server.registerTool(
      codeFunctionDecompileToolDefinition,
      createCodeFunctionDecompileHandler(wm, db)
    )
    server.registerTool(
      codeFunctionDisassembleToolDefinition,
      createCodeFunctionDisassembleHandler(wm, db)
    )
    server.registerTool(codeFunctionCFGToolDefinition, createCodeFunctionCFGHandler(wm, db))
    server.registerTool(
      codeFunctionsReconstructToolDefinition,
      createCodeFunctionsReconstructHandler(wm, db, cm)
    )
    server.registerTool(
      codeFunctionRenamePrepareToolDefinition,
      createCodeFunctionRenamePrepareHandler(wm, db, cm)
    )
    server.registerTool(
      codeFunctionExplainPrepareToolDefinition,
      createCodeFunctionExplainPrepareHandler(wm, db, cm)
    )
    server.registerTool(
      codeFunctionExplainApplyToolDefinition,
      createCodeFunctionExplainApplyHandler(wm, db)
    )
    server.registerTool(
      codeModuleReviewPrepareToolDefinition,
      createCodeModuleReviewPrepareHandler(wm, db, cm)
    )
    server.registerTool(
      codeModuleReviewApplyToolDefinition,
      createCodeModuleReviewApplyHandler(wm, db)
    )
    server.registerTool(
      codeFunctionRenameApplyToolDefinition,
      createCodeFunctionRenameApplyHandler(wm, db)
    )
    server.registerTool(
      codeReconstructExportToolDefinition,
      createCodeReconstructExportHandler(wm, db, cm)
    )
    server.registerTool(
      dotNetReconstructExportToolDefinition,
      createDotNetReconstructExportHandler(wm, db, cm)
    )
    server.registerTool(
      codeReconstructPlanToolDefinition,
