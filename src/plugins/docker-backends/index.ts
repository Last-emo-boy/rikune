...
const dockerBackendsPlugin: Plugin = {
  ...
  systemDeps: [
    ...
    { type: 'python-venv', name: 'angr', target: '$ANGR_PYTHON', envVar: 'ANGR_PYTHON', dockerDefault: '/opt/angr-venv/bin/python', required: false, description: 'angr symbolic execution (venv)', dockerInstall: 'python3 -m venv /opt/angr-venv && pip install angr', dockerFeature: 'angr', dockerValidation: ['/opt/angr-venv/bin/python -c "import angr; print(\'✓ angr\')"'], buildArgs: { ANGR_VERSION: '9.2.205' } },
    {
      ...
      volumes: [{ source: '${RIKUNE_DATA_ROOT:-D:/Docker/rikune}/qiling-rootfs', target: '/opt/qiling-rootfs', mode: 'ro' }],
    },
    ...
    { type: 'python', name: 'pandare', importName: 'pandare', required: false, description: 'PANDA record/replay analysis', dockerInstall: 'pip install pandare', dockerFeature: 'dynamic-python', extraEnv: { PANDA_PYTHON: '/usr/local/bin/python3', YARAX_PYTHON: '/usr/local/bin/python3' } },
