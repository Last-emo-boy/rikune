...
function collectFindings(payloads: CryptoIdentificationPayload[]): CryptoFinding[] {
  return payloads.flatMap((payload) =>
    Array.isArray(payload.algorithms) ? payload.algorithms : []
  )
}

function collectPayloadConstants(
  payloads: CryptoIdentificationPayload[]
): CryptoConstantCandidate[] {
  return payloads.flatMap((payload) =>
    Array.isArray(payload.candidate_constants) ? payload.candidate_constants : []
  )
}
...
function buildGraph(options: {
  ...
  const runtimeApiSet = new Set(
    options.runtimeApis.map((api) => normalizeApiName(api).toLowerCase())
  )
    ...
    finding.source_apis.forEach((api) => {
      ...
      if (runtimeApiSet.has(normalized.toLowerCase())) {
        ...
        addEdge(edges, {
          from: apiNodeId,
          to: algorithmNodeId,
          label: 'corroborates_crypto_path',
          confidence: Math.max(0.8, finding.confidence),
        })
      }
    })
  ...
  payloadConstants.forEach((constant, index) => {
    ...
    addEdge(edges, {
      from: sampleNodeId,
      to: constantNodeId,
      label: 'has_crypto_constant_candidate',
      confidence: 0.72,
    })
  })

  if (
    options.runtimeApis.length > 0 ||
    options.runtimeStages.length > 0 ||
    options.runtimeMemoryRegions.length > 0
  ) {
  ...
  return async (args: unknown): Promise<WorkerResult> => {
    ...
    try {
      ...
      if (cryptoSelection.artifacts.length === 0) {
        warnings.push(
          'No crypto_identification artifacts were found. Run crypto.identify first for a richer lifecycle graph.'
        )
      }
      if (!runtimeEvidence) {
        warnings.push(
          'No dynamic trace artifacts were found; graph is limited to static crypto evidence.'
        )
      }
      ...
      const data = {
        ...
        next_actions:
          graph.corroborated_apis.length > 0
            ? [
