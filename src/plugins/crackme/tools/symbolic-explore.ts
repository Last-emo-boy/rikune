...
export const SymbolicExploreInputSchema = z.object({
  ...
  find_addresses: z
    .array(z.string())
    .min(1)
    .describe('Target addresses to reach (hex, e.g. ["0x401234"])'),
  avoid_addresses: z.array(z.string()).optional().default([]).describe('Addresses to avoid (hex)'),
  start_address: z
    .string()
    .optional()
    .describe('Custom start address instead of entry point (hex)'),
  input_length: z
    .number()
    .int()
    .min(1)
    ...
    .optional()
    .default(32)
    .describe('Symbolic input length in bytes'),
  timeout_sec: z
    .number()
    .int()
    .min(5)
...
export const SymbolicExploreOutputSchema = createWorkerResultOutputSchema(
  ...
  return async (args: z.infer<typeof SymbolicExploreInputSchema>): Promise<WorkerResult> => {
    ...
    try {
      ...
      if (!policyDecision.allowed) {
        return {
          ok: false,
          errors: [policyDecision.reason || 'Symbolic execution denied by policy guard.'],
