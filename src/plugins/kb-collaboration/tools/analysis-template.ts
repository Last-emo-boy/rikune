...
const TEMPLATES: Record<string, TemplateStep[]> = {
  malware_triage: [
    ...
    {
      tool: 'pe.fingerprint',
      description: 'Basic PE fingerprint (hashes, header)',
      required: true,
    },
    { tool: 'compiler.packer.detect', description: 'Detect compiler and packer', required: true },
    {
      tool: 'static.capability.triage',
      description: 'Identify capabilities (network, crypto, anti-debug)',
      required: true,
    },
    ...
    {
      tool: 'c2.extract',
      description: 'Extract C2 indicators',
      required: false,
      depends_on: ['strings.extract'],
    },
    { tool: 'malware.classify', description: 'Classify malware family', required: false },
    {
      tool: 'attack.map',
      description: 'Map to MITRE ATT&CK',
      required: false,
      depends_on: ['static.capability.triage'],
    },
  ...
  malware_deep: [
    ...
    {
      tool: 'unpack.auto',
      description: 'Auto-unpack if packed',
      required: false,
      depends_on: ['compiler.packer.detect'],
    },
    ...
    { tool: 'strings.extract', description: 'String extraction', required: true },
    {
      ...
      description: 'Decode obfuscated strings (FLOSS)',
      required: false,
    },
    ...
    {
      tool: 'sandbox.report',
      description: 'Sandbox behavior report',
      required: false,
      depends_on: ['sandbox.execute'],
    },
    {
  ...
  crackme_solve: [
    ...
    { tool: 'compiler.packer.detect', description: 'Check for packing', required: true },
    {
      ...
      description: 'Extract strings (success/failure messages)',
      required: true,
    },
    ...
    {
      tool: 'crackme.locate.validation',
      description: 'Locate validation function',
      required: true,
      depends_on: ['ghidra.analyze'],
    },
    {
      tool: 'code.function.decompile',
      description: 'Decompile validation function',
      required: true,
      depends_on: ['crackme.locate.validation'],
      args_hint: { function_address: '<from locate.validation>' },
    },
    {
      tool: 'constraint.extract',
      description: 'Extract input constraints from validation',
      required: false,
    },
    {
      tool: 'smt.solve',
      description: 'Solve constraints for valid key',
      required: false,
      depends_on: ['constraint.extract'],
    },
    {
      tool: 'symbolic.explore',
      description: 'Symbolic execution to find success path',
      required: false,
    },
    {
      tool: 'patch.generate',
      description: 'Generate bypass patch',
      required: false,
      depends_on: ['crackme.locate.validation'],
    },
  ...
  packer_unpack: [
    ...
    {
      tool: 'unpack.auto',
      description: 'Auto-unpack attempt',
      required: true,
      depends_on: ['compiler.packer.detect'],
    },
    {
      tool: 'strings.extract',
      description: 'Extract strings from unpacked',
      required: false,
      depends_on: ['unpack.auto'],
    },
    {
      tool: 'pe.imports.extract',
      description: 'Recover import table',
      required: false,
      depends_on: ['unpack.auto'],
    },
  ],
  vulnerability_audit: [
    ...
    { tool: 'ghidra.analyze', description: 'Ghidra analysis', required: true },
    {
      tool: 'vuln.pattern.scan',
      description: 'Scan for vulnerability patterns',
      required: true,
      depends_on: ['ghidra.analyze'],
    },
    {
      tool: 'vuln.pattern.summary',
      description: 'Summarize findings',
      required: false,
      depends_on: ['vuln.pattern.scan'],
    },
