...
describe('code.functions.define tool', () => {
  ...
  test('should import recovered functions, persist an artifact, and keep decompile/cfg readiness missing', async () => {
    ...
    expect(data.boundary_application_scope).toBe('rikune_function_index_only')
    expect(data.ghidra_project_updated).toBe(false)
    expect(result.warnings?.join(' ')).toContain('read-only Ghidra project was not updated')
    ...
    expect(output.boundary_application_scope).toBe('rikune_function_index_only')
    expect(output.ghidra_project_updated).toBe(false)
  })
})
