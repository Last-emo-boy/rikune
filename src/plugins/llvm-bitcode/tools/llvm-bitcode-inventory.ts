...
const LlvmBitcodeInventorySchema = z.object({
  ...
  bitstream: z.record(z.string(), z.any()),
  llvm_ir_hints: z.record(z.string(), z.any()),
  embedded_strings: z.record(z.string(), z.any()),
...
function hasRawBitcodeMagic(data: Buffer, offset = 0): boolean {
  return (
    data.length >= offset + RAW_MAGIC.length && data.subarray(offset, offset + 4).equals(RAW_MAGIC)
  )
}
...
function parseWrapper(data: Buffer): Record<string, unknown> {
  ...
  const embeddedMagicValid =
    boundsValid && offset !== null ? hasRawBitcodeMagic(data, offset) : false
  return {
    ...
    trailing_bytes:
      declaredEnd !== null && declaredEnd <= data.length ? data.length - declaredEnd : null,
  }
}
...
function parseBitstream(
  ...
  streamOffset: number,
  declaredSize?: number
): BitstreamParseResult {
  const bytesAvailable = Math.max(
    0,
    Math.min(data.length - streamOffset, declaredSize ?? data.length)
  )
...
function buildStringHints(strings: string[]) {
  ...
  const triples = strings
    .filter((value) =>
      /(?:x86_64|aarch64|arm|wasm32|riscv|powerpc|mips).*(?:linux|windows|darwin|apple|unknown|wasi)/i.test(
        value
      )
    )
    .slice(0, 24)
...
function buildIrHints(strings: string[], bitstream: BitstreamParseResult) {
  return {
    target_triples: strings
      ...
      .slice(0, 16),
    source_filenames: strings
      .filter((value) => /\.(c|cc|cpp|cxx|m|mm|rs|swift|ll)$/i.test(value))
      .slice(0, 16),
    section_name_hints: strings
      .filter((value) => /^(\.|__LLVM|llvmbc|llvm\.lto)/i.test(value))
      .slice(0, 16),
    module_block_present: bitstream.block_summaries.some((block) => block.block_id === 8),
    function_block_count_hint: bitstream.block_summaries.filter((block) => block.block_id === 12)
      .length,
    metadata_block_count_hint: bitstream.block_summaries.filter((block) => block.block_id === 15)
      .length,
...
function buildRiskFlags(input: {
  ...
  if (input.format === 'unknown') {
    flags.push(
      riskFlag(
        ...
        'medium',
        'format',
        'No LLVM bitcode magic or strong LLVM hint was found.'
      )
    )
  }
  if (input.confidence === 'medium') {
    flags.push(
      riskFlag(
        'extension-only-unverified',
        'low',
        'format',
        'LLVM bitcode classification relies on extension fallback.'
      )
    )
  }
    ...
    if (input.container.bounds_valid !== true) {
      flags.push(
        riskFlag(
          ...
          'high',
          'structure',
          'Wrapper offset/size does not fit inside the preview.'
        )
      )
    }
    if (input.container.version !== 0) {
      flags.push(
        riskFlag(
          ...
          'medium',
          'structure',
          'Wrapper version is not the expected value 0.'
        )
      )
    }
    if (input.container.embedded_magic_valid !== true) {
      flags.push(
        riskFlag(
          ...
          'high',
          'structure',
          'Wrapper payload does not start with raw LLVM bitcode magic.'
        )
      )
    }
  }
  if (
    ...
    flags.push(
      riskFlag(
        ...
        'medium',
        'parser',
        `Bitstream parser status is ${input.bitstream.decode_status}.`
      )
    )
  }
  if (input.bitstream.decode_status === 'limit-hit') {
    flags.push(
      riskFlag(
        'parser-limit-reached',
        'medium',
        'parser',
        'Bitstream parser hit the safety step limit.'
      )
    )
  }
  if (input.strings.paths.length > 0 || input.strings.urls.length > 0) {
    flags.push(
      riskFlag(
        'embedded-path-or-url',
        'low',
        'privacy',
        'Bitcode preview contains path or URL-like string evidence.'
      )
    )
  }
  if (input.format === 'llvm-bitcode-native-object-hint') {
    flags.push(
      riskFlag(
        'native-object-bitcode-hint',
        'low',
        'routing',
        'Native object container hints should be reviewed with native.object.inventory.'
      )
    )
  }
  return flags
...
export function createLlvmBitcodeInventoryHandler(deps: PluginToolDeps) {
  ...
  return async (args: z.infer<typeof LlvmBitcodeInventoryInputSchema>): Promise<WorkerResult> => {
    const t0 = Date.now()
    try {
