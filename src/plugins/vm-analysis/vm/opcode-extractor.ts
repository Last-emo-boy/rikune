...
export function extractSwitchCases(decompiledCode: string): CaseEntry[] {
  ...
  const caseRe =
    /\bcase\s+(0x[\da-fA-F]+|\d+)\s*:([\s\S]*?)(?=\bcase\s+(?:0x[\da-fA-F]+|\d+)\s*:|\bdefault\s*:|}\s*$)/g
...
const HANDLER_PATTERNS: PatternDef[] = [
  ...
  // Control
  {
    mnemonic: 'JMP',
    category: 'control',
    patterns: [/\bpc\s*=\s*/, /\bip\s*=\s*/, /\bgoto\b/],
    confidence: 80,
  },
  {
    mnemonic: 'JZ',
    category: 'control',
    patterns: [/if\s*\(\s*\w+\s*==\s*0\s*\)\s*\{?\s*\w*(pc|ip)\w*\s*=/],
    confidence: 75,
  },
  {
    mnemonic: 'JNZ',
  ...
  // Stack
  {
    mnemonic: 'PUSH',
    category: 'stack',
    patterns: [/\bsp\b.*--|\bstack\b.*\bsp\b/i],
    confidence: 70,
  },
  {
    mnemonic: 'POP',
  ...
  // Compare
  {
    mnemonic: 'CMP',
    category: 'compare',
    patterns: [/\bif\s*\(\s*\w+\s*[=!<>]=?\s*\w+\s*\)/, /\bflags?\b.*=.*[<>=!]/i],
    confidence: 75,
  },
  {
    mnemonic: 'TEST',
    category: 'compare',
    patterns: [/\bif\s*\(\s*\w+\s*&\s*\w+\s*\)/],
    confidence: 70,
  },
  ...
  // Special
  {
    mnemonic: 'NOP',
    category: 'special',
    patterns: [/^\s*;?\s*$/, /\bbreak\s*;?\s*$/],
    confidence: 60,
  },
  {
    mnemonic: 'HALT',
    category: 'special',
    patterns: [/\breturn\b/, /\bexit\b/, /\brunning\s*=\s*(0|false)\b/],
    confidence: 70,
  },
  {
    mnemonic: 'SYSCALL',
    category: 'special',
    patterns: [/\bcall\b|\bfunc_\w+\s*\(/, /\binvoke\b/i],
    confidence: 60,
  },
]
