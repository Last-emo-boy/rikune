...
import {
  behaviorCaptureToolDefinition,
  createBehaviorCaptureHandler,
} from './tools/behavior-capture.js'
import { behaviorIocToolDefinition, createBehaviorIocHandler } from './tools/behavior-ioc.js'
import {
  behaviorNetworkToolDefinition,
  createBehaviorNetworkHandler,
} from './tools/behavior-network.js'
...
const behaviorFirstPlugin: Plugin = {
  ...
  aspects: {
    formats: ['pe', 'dll', 'dotnet', 'elf', 'so', 'macho', 'apk', 'ipa'],
    platforms: ['windows', 'linux', 'macos', 'ios', 'android'],
    architectures: ['x86', 'x64', 'arm', 'arm64'],
    execution: ['dynamic', 'correlation'],
    runtimes: [
      'windows-sandbox',
      'hyperv',
      'wine',
    ...
    evidence: ['process', 'filesystem', 'registry', 'network', 'memory', 'timeline', 'ioc'],
  },
  runtimePolicy: {
    ...
    notes: [
      'Behavior capture is runtime-gated; IOC and network post-processing may run only on imported evidence.',
      'Default readiness must not start a sandbox, emulator, debugger, or Frida session.',
    ],
  },
  surfaceRules: {
    tier: 2,
    activateOn: { findings: ['c2', 'suspicious_imports', 'anti_debug'] },
  ...
  configSchema: [
    {
      envVar: 'BEHAVIOR_TIMEOUT',
      description: 'Default behavioral capture timeout in seconds',
      required: false,
      defaultValue: '60',
    },
  ],
