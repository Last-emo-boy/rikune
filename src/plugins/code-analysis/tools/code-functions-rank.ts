...
export const codeFunctionsRankInputSchema = z.object({
  ...
  include_vuln_risk: z
    ...
    .optional()
    .describe(
      'When true, boost ranking score based on vulnerability findings from vuln.pattern.scan'
    ),
})
      ...
      if (input.include_vuln_risk) {
        ...
        if (vulnArtifacts.length > 0) {
          try {
            ...
            const findings: Array<{ function_name: string; severity: string }> =
              vulnData?.findings ?? []
            ...
            for (const fn of rankedFunctions as Array<{
              name?: string
              score?: number
              vuln_risk_boost?: number
            }>) {
            ...
            ;(rankedFunctions as Array<{ score?: number }>).sort(
