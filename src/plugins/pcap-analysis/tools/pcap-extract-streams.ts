...
export const pcapExtractStreamsInputSchema = z.object({
  ...
  stream_index: z
    ...
    .min(0)
    .optional()
    .describe('Specific TCP stream index to extract. Omit for summary of all streams.'),
      ...
      if (input.stream_index !== undefined) {
        ...
        if (input.persist_artifact && result.stdout.length > 0) {
          artifact = await persistBackendArtifact(
            ...
            input.sample_id,
            'pcap',
            `stream-${input.protocol}-${input.stream_index}`,
            preview,
        ...
        for (const line of lines) {
          ...
          const m = line.match(
            /^([\d.:[\]a-fA-F]+)\s+<->\s+([\d.:[\]a-fA-F]+)\s+\d+\s+(\d+)\s+\d+\s+(\d+)\s+\d+\s+(\d+)/
          )
          if (m) {
