...
The repository currently contains 102 built-in plugins.
...
| Linux ELF, SO, core, modules, packages, eBPF, BTF, DWARF/CTF | `linux-binary`, `linux-package`, `elf-macho`, `native-object`, `native-debug-types`, `container-analysis`, `btf`, `ebpf-bytecode` | `linux-runtime`, `qiling`, `debug-session`, `behavior-first` | No ELF execution, ptrace, kernel module loading, package install, eBPF load/attach, BTF verifier submission, DWARF/CTF external dumpers, symbol-server download, source fetch, or eBPF collection by default. |
| macOS Mach-O, app bundles, frameworks, DMG, PKG, dSYM | `apple-container`, `apple-signing`, `apple-objc-swift`, `elf-macho`, `native-object`, `native-debug-types` | `macos-runtime`, `debug-session`, `frida`, `behavior-first` | No DMG mount, app launch, LLDB attach, DTrace, fs_usage capture, Objective-C runtime attach, Swift demangle process, DWARF external dumpers, source fetch, or external Apple tooling by default. |
...
| Advanced native lifting, symbolic execution, IR, debug/type metadata, GPU artifacts, and backend comparison workflows | `revng`, `triton`, `miasm`, `lief`, `radare2`, `remill`, `gtirb`, `manifold`, `native-debug-types`, `llvm-bitcode`, `shader-ir`, `cuda-binary`, `culifter`, `vm-analysis`, `rizin`, `ghidra`, `retdec` | Worker-backed GTIRB, Remill, Manifold, QBDI, and CuLifter surfaces are bounded and readiness-gated; runtime/emulation remains opt-in | Discovery/readiness/help/list paths emit backend plans and readiness metadata only; no LLVM toolchain, debug dumper, heavy backend process, solver, emulator, fact engine, binary mutation, shader compiler, CUDA driver, GPU access, or sample execution starts during discovery. |
