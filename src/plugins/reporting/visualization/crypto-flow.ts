...
export interface CryptoFlowOptions {
  maxNodes?: number
  format?: 'dot' | 'mermaid' | 'json'
  sampleId?: string
}

const CRYPTO_TEMPLATES: Record<
  string,
  { apis: string[]; flow: Array<{ from: string; to: string; label: string }> }
> = {
  AES: {
    apis: ['CryptEncrypt', 'CryptDecrypt', 'BCryptEncrypt', 'BCryptDecrypt'],
    flow: [
      { from: 'Key Generation', to: 'Key Schedule', label: 'expand key' },
      { from: 'Key Schedule', to: 'Encryption Rounds', label: 'round keys' },
      { from: 'Encryption Rounds', to: 'Ciphertext', label: 'output' },
    ],
  },
  RC4: {
    apis: ['CryptEncrypt', 'CryptDecrypt', 'RC4', 'ARC4'],
    flow: [
      { from: 'Key Input', to: 'KSA', label: 'initialize' },
      { from: 'KSA', to: 'PRGA', label: 'generate keystream' },
      { from: 'PRGA', to: 'XOR', label: 'keystream' },
      { from: 'Plaintext', to: 'XOR', label: 'input' },
      { from: 'XOR', to: 'Ciphertext', label: 'output' },
    ],
  },
  RSA: {
    apis: ['CryptEncrypt', 'CryptDecrypt', 'BCryptEncrypt', 'BCryptDecrypt'],
    flow: [
      { from: 'Key Pair Generation', to: 'Public Key', label: 'extract' },
      { from: 'Key Pair Generation', to: 'Private Key', label: 'extract' },
      { from: 'Public Key', to: 'Encryption', label: 'encrypt' },
      { from: 'Private Key', to: 'Decryption', label: 'decrypt' },
    ],
  },
  Generic: {
    apis: ['CryptGenKey', 'CryptImportKey', 'CryptExportKey'],
    flow: [
      { from: 'Key Generation', to: 'Encryption', label: 'key' },
      { from: 'Plaintext', to: 'Encryption', label: 'input' },
      { from: 'Encryption', to: 'Ciphertext', label: 'output' },
    ],
  },
}

export function generateCryptoFlow(
  cryptoFindings: Array<{
    algorithm?: string
    confidence: number
    functions?: Array<{ address: string; name: string; apis?: string[] }>
  }>,
  options: CryptoFlowOptions = {}
): CryptoFlowData {
  const { maxNodes = 20 } = options
  const nodes: CryptoFlowNode[] = []
  const edges: CryptoFlowEdge[] = []
  ...
  for (const finding of cryptoFindings.slice(0, maxNodes)) {
    ...
    nodes.push({
      ...
      algorithm,
      isSuspicious: finding.confidence > 0.7,
  ...
  const confidence =
    cryptoFindings.length > 0
      ? cryptoFindings.reduce((sum, f) => sum + f.confidence, 0) / cryptoFindings.length
      : 0
  const explanation = ExplanationGraphDigestSchema.parse({
    ...
    confidence_state: cryptoFindings.some((item) => item.confidence >= 0.85)
      ? 'correlated'
      : 'inferred',
    ...
    provenance: [
      ...
      },
      {
        ...
        detail:
          'Flow edges are template-driven and should be treated as inferred structure unless separately observed at runtime.',
      },
    ],
    omissions: [
      {
        ...
        reason:
          'The graph emphasizes bounded algorithm flow explanation and omits low-level block-by-block implementation detail.',
      },
...
export function cryptoFlowToDot(data: CryptoFlowData, options?: { title?: string }): string {
  ...
  for (const node of data.nodes) {
    ...
    if (node.isSuspicious) {
      color = 'red'
      style = style.includes('filled') ? `${style},bold` : 'filled,bold'
    }
...
export function cryptoFlowToMermaid(data: CryptoFlowData): string {
  ...
  mermaid += '\n'
  for (const edge of data.edges)
    mermaid += `  ${edge.source} -->|${escapeMermaid(edge.label || '')}| ${edge.target}\n`
  return (
    mermaid +
