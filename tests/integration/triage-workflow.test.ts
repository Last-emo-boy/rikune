...
describe('Triage Workflow Integration', () => {
  ...
  beforeAll(async () => {
    ...
    ingestHandler = createSampleIngestHandler(
      workspaceManager,
      database,
      policyGuard,
      sampleOperationGate
  ...
  test(
    'should detect suspicious strings in sample',
    async () => {
      // Requirement: 15.5
  ...
  test(
    'should handle multiple samples concurrently',
    async () => {
      ...
      const startTime = Date.now()
      const results = await Promise.all(sampleIds.map((id) => triageHandler({ sample_id: id })))
      const elapsedMs = Date.now() - startTime
      ...
      const summaries = results.filter((r) => r.ok && r.data).map((r) => (r.data as any).summary)
  ...
  test(
    'should provide appropriate threat assessment',
    async () => {
  ...
  test(
    'should handle partial tool failures gracefully',
    async () => {
  ...
  test(
    'should cache results for repeated analysis',
    async () => {
