#===== STAGE =====
# ── de4dot build stage ──
FROM mcr.microsoft.com/dotnet/sdk:3.1 AS de4dot-build
...
RUN apt-get update && apt-get install -y --no-install-recommends git && \
    ...
    dotnet publish de4dot/de4dot.csproj -c Release -f netcoreapp3.1 -o /opt/de4dot && \
    rm -rf /tmp/de4dot && \
...
RUN ln -sf /opt/de4dot/de4dot /usr/local/bin/de4dot || \
    ln -sf /opt/de4dot/de4dot.dll /usr/local/bin/de4dot.dll
