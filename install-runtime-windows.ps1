...
function Invoke-RuntimePrivateEnvSnapshotOperation {
    ...
    try {
        ...
        if ($nativeExitCode -ne 0) {
            ...
            if (
                $nativeFailureText -match '(?im)^\s*(?:node(?:\.exe)?\s*:\s*)?Error:\s+RIKUNE_PRIVATE_ENV_FAILURE=(acl-(?:snapshot-match|pre-unlink)-child-other):\s+Unable to verify Windows ACL \(child-status=(0xFFFF0000); child-stderr=(silent|ascii-like|ascii-nul-le-shape|opaque|legacy-en-clr-load-shape|legacy-en-clr-start-shape|legacy-en-appdomain-shape|legacy-en-managed-entry-shape|legacy-en-consolehost-startup-shape)\)[ \t]*\r?$'
            ) {
