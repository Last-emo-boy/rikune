...
const EbpfInventorySchema = z.object({
  ...
  elf_header: z.record(z.any()).optional(),
  sections: z.array(z.record(z.any())),
  instructions: z.array(EbpfInstructionSchema),
  helper_calls: z.array(z.record(z.any())),
  map_references: z.array(z.record(z.any())),
  control_flow: z.record(z.any()),
  verifier_precheck: z.record(z.any()),
...
export function createEbpfBytecodeInventoryHandler(deps: PluginToolDeps) {
  const { workspaceManager, database, resolvePrimarySamplePath, persistStaticAnalysisJsonArtifact } = deps
