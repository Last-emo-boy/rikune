...
describe('JobQueue', () => {
  ...
  describe('concurrency control', () => {
    ...
    it('should handle mixed concurrent operations (complete, fail, cancel)', () => {
      ...
      expect(queue.getStatus(jobId3)?.status).toBe('cancelling')
      queue.complete(jobId3, {
        jobId: jobId3,
        ok: false,
        errors: ['cancelled'],
        warnings: [],
      ...
      expect(queue.getStatus(jobId3)?.status).toBe('interrupted')
    })
  ...
  describe('retry wait fencing', () => {
    it('does not let the stale-running reaper strand a backoff job', async () => {
      const jobId = queue.enqueue({
        type: 'static',
        tool: 'pe.fingerprint',
        sampleId: 'sha256:test',
        ...
        priority: JobPriority.NORMAL,
        timeout: 30000,
        retryPolicy: { maxRetries: 3, backoffMs: 20, retryableErrors: ['E_TIMEOUT'] },
      })
      queue.dequeue()
      queue.complete(jobId, {
        jobId,
      ...
      expect(queue.getStatus(jobId)?.status).toBe('retry_wait')
      expect(queue.reapStaleRunningJobs(0, Date.now() + 1000)).toEqual([])
      await new Promise((resolve) => setTimeout(resolve, 60))
      expect(queue.getStatus(jobId)?.status).toBe('queued')
      expect(queue.getQueueLength()).toBe(1)
    })

    it('cancels the retry token so a backoff job cannot resurrect', async () => {
      const jobId = queue.enqueue({
        type: 'static',
        tool: 'pe.fingerprint',
        sampleId: 'sha256:test',
      ...
      expect(queue.cancel(jobId, 'cancel during retry wait')).toBe(true)
      await new Promise((resolve) => setTimeout(resolve, 60))
      expect(queue.getStatus(jobId)?.status).toBe('cancelled')
      expect(queue.getQueueLength()).toBe(0)
      expect(() => queue.requeue(running)).toThrow(/E_JOB_REQUEUE_STATE/)
    })
  })
})
