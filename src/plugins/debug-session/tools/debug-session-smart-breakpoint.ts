...
export const DebugSessionSmartBreakpointInputSchema = z.object({
  ...
  strategy: z
    ...
    .default('auto')
    .describe('Breakpoint strategy category'),
  max_breakpoints: z
    .number()
    .int()
    .min(1)
...
const STRATEGY_RULES: Record<string, BreakpointRule[]> = {
  crypto_intercept: [
    {
      category: 'crypto',
      api_pattern: 'CryptEncrypt',
      description: 'Windows CryptoAPI encrypt',
      priority: 1,
    },
    {
      category: 'crypto',
      api_pattern: 'CryptDecrypt',
      description: 'Windows CryptoAPI decrypt',
      priority: 1,
    },
    {
      category: 'crypto',
      api_pattern: 'BCryptEncrypt',
      description: 'BCrypt encrypt',
      priority: 1,
    },
    {
      category: 'crypto',
      api_pattern: 'BCryptDecrypt',
      description: 'BCrypt decrypt',
      priority: 1,
    },
    {
      category: 'crypto',
      api_pattern: 'CryptHashData',
      description: 'Hash data input',
      priority: 2,
    },
    {
      category: 'crypto',
      api_pattern: 'CryptDeriveKey',
  ...
  network_monitor: [
    ...
    {
      category: 'network',
      api_pattern: 'URLDownloadToFileA',
  ...
  unpack_oep: [
    {
      category: 'unpack',
      api_pattern: 'VirtualAlloc',
      description: 'Memory allocation for unpacking',
      priority: 1,
      condition: 'size > 0x1000',
    },
    {
      category: 'unpack',
      api_pattern: 'VirtualProtect',
      description: 'Memory protection change (RWX)',
      priority: 1,
      condition: 'newProtect == 0x40',
    },
    {
      category: 'unpack',
      api_pattern: 'WriteProcessMemory',
      description: 'Remote process write',
      priority: 1,
    },
    {
      category: 'unpack',
      api_pattern: 'NtUnmapViewOfSection',
  ...
  anti_debug: [
    {
      category: 'anti_debug',
      api_pattern: 'IsDebuggerPresent',
      description: 'Debugger check',
      priority: 1,
    },
    {
      category: 'anti_debug',
      api_pattern: 'CheckRemoteDebuggerPresent',
      description: 'Remote debugger check',
      priority: 1,
    },
    {
      category: 'anti_debug',
      api_pattern: 'NtQueryInformationProcess',
      description: 'Process info query (debug port)',
      priority: 1,
    },
    {
      category: 'anti_debug',
      api_pattern: 'OutputDebugStringA',
      description: 'Debug string output (timing check)',
      priority: 2,
    },
    {
      category: 'anti_debug',
      api_pattern: 'GetTickCount',
      description: 'Timing-based anti-debug',
      priority: 2,
    },
    {
      category: 'anti_debug',
      api_pattern: 'QueryPerformanceCounter',
      description: 'High-res timing check',
      priority: 2,
    },
  ],
  string_decrypt: [
    {
      ...
      api_pattern: 'VirtualAlloc',
      description: 'Buffer for decrypted strings',
      priority: 2,
    },
    {
      category: 'string_decrypt',
      api_pattern: 'RtlDecompressBuffer',
      description: 'Decompression (string table)',
      priority: 1,
    },
    {
      category: 'string_decrypt',
      api_pattern: 'MultiByteToWideChar',
      description: 'String conversion post-decrypt',
      priority: 2,
    },
  ],
}
...
export function createDebugSessionSmartBreakpointHandler(deps: PluginToolDeps) {
  ...
  return async (args: Record<string, unknown>): Promise<WorkerResult> => {
    ...
    try {
      ...
      const importArtifact = artifacts.find(
        (a: { type: string }) => a.type === 'imports' || a.type === 'pe_imports'
      )

      // Determine which strategies to apply
      const strategies = input.strategy === 'auto' ? Object.keys(STRATEGY_RULES) : [input.strategy]
