  ...
  options: z
    .object({
      top_functions: z
        .number()
        .optional()
        .describe('Number of top functions to decompile (default: 10)'),
      ghidra_timeout: z
        .number()
        .optional()
        .describe('Ghidra analysis timeout in seconds (default: 600)'),
      include_cfg: z
        .boolean()
        .optional()
        .describe('Include CFG for top functions (default: false)'),
    })
    .optional(),
})
...
function buildDeepStaticCoverage(params: {
  ...
  return buildCoverageEnvelope({
    ...
    knownFindings: [
      params.functionCount !== undefined
        ? `Recovered ${params.functionCount} functions during deep static analysis.`
        : null,
      !params.queued && decompiledCount > 0
        ? `Decompiled ${decompiledCount} high-value function(s).`
        : null,
    ],
    suspectedFindings: [
      ...
      !params.queued
        ? {
            ...
            expected_coverage_gain:
              'Extends decompilation coverage to functions not included in the initial top-function batch.',
            cost_tier: 'medium',
          }
        : null,
    ],
      ...
      return buildJsonResult({
        ...
        data: mergeCoverageEnvelope(
          ...
          buildDeepStaticCoverage({
            ...
            queued: false,
            functionCount:
              typeof resultData.function_count === 'number' ? resultData.function_count : undefined,
