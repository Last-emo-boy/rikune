...
import {
  ...
  createDynamicTraceAttributeHandler,
} from './tools/dynamic-trace-attribute.js'
import {
  dynamicMemoryDumpToolDefinition,
  createDynamicMemoryDumpHandler,
...
const dynamicPlugin: Plugin = {
  ...
  version: '1.0.0',
  configSchema: [{ envVar: 'FRIDA_PATH', description: 'Path to frida CLI', required: false }],
  systemDeps: [
    {
      type: 'binary',
      name: 'frida',
  ...
  register(server, deps) {
    ...
    server.registerTool(
      dynamicTraceAttributeToolDefinition,
      createDynamicTraceAttributeHandler(deps)
    )
    ...
    server.registerTool(
      runtimeDebugSessionStartToolDefinition,
      createRuntimeDebugSessionStartHandler(deps)
    )
    server.registerTool(
      runtimeDebugSessionStatusToolDefinition,
      createRuntimeDebugSessionStatusHandler(deps)
    )
    server.registerTool(
      runtimeDebugSessionStopToolDefinition,
      createRuntimeDebugSessionStopHandler(deps)
    )
    ...
    server.registerTool(
      dynamicBehaviorCaptureToolDefinition,
      createDynamicBehaviorCaptureHandler(deps)
    )
