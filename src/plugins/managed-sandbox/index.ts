...
import { safeRunToolDefinition, createSafeRunHandler } from './tools/safe-run.js'
...
const managedSandboxPlugin: Plugin = {
  ...
  configSchema: [
    {
      envVar: 'SANDBOX_PYTHON_PATH',
      description: 'Python interpreter for the sandbox worker',
      required: false,
    },
    {
      envVar: 'SANDBOX_TIMEOUT',
      description: 'Default execution timeout in seconds (1–300)',
      required: false,
      defaultValue: '60',
    },
    {
      envVar: 'SANDBOX_MEMORY_MB',
      description: 'Default memory limit in MB (32–2048)',
      required: false,
      defaultValue: '512',
    },
    {
      envVar: 'SANDBOX_NETWORK_SINKHOLE',
      description: 'Enable network sinkhole by default (true/false)',
      required: false,
      defaultValue: 'true',
    },
  ],
  systemDeps: [
    {
      type: 'binary',
      name: 'python3',
      target: '$SANDBOX_PYTHON_PATH',
      envVar: 'SANDBOX_PYTHON_PATH',
      versionFlag: '--version',
      dockerDefault: '/usr/local/bin/python3',
      required: true,
      description: 'Python 3 interpreter for sandbox worker',
    },
    {
      type: 'binary',
      name: 'dotnet',
      ...
      envVar: 'DOTNET_PATH',
      versionFlag: '--info',
      dockerDefault: '/usr/bin/dotnet',
      required: false,
      description: '.NET runtime for managed assembly execution',
