...
$nativeDiagnosticCases = @(
    ...
    @{ Snapshot = "snapshot-secret-acl-init-managed-entry"; Category = "acl-snapshot-match-child-other"; ExitCode = 97; Phase = "verifier-diagnostic"; UseRemoveWrapper = $false; ChildStatus = "0xFFFF0000"; ChildStderr = "legacy-en-managed-entry-shape" },
...
process.stdin.on('end', () => {
  ...
  const cases = {
    ...
    'snapshot-secret-acl-init-managed-entry': [97, `Error: RIKUNE_PRIVATE_ENV_FAILURE=acl-snapshot-match-child-other: Unable to verify Windows ACL (child-status=0xFFFF0000; child-stderr=legacy-en-managed-entry-shape)\nError: ignored '${mode}-child-secret-marker'\n`],
