...
} from '../../static-triage/tools/binary-role-profile.js'
import {
  RustBinaryAnalyzeDataSchema,
  createRustBinaryAnalyzeHandler,
} from '../../static-triage/tools/rust-binary-analyze.js'
...
import { buildArtifactSelectionDiff } from '../../../artifacts/selection-diff.js'
import {
...
export const reportGenerateInputSchema = z
  .object({
    ...
    evidence_scope: z
      ...
      .default('all')
      .describe(
        'Runtime evidence scope: all artifacts, latest artifact window, or a specific session selector'
      ),
    ...
    compare_static_scope: z
      ...
      .optional()
      .describe(
        'Optional baseline static-analysis scope used to compare capability, PE structure, and compiler/packer selections'
      ),
...
function generateMarkdownReport(
  ...
  if (functions.length > 0) {
    ...
    if (topFunctions.length > 0) {
      ...
      topFunctions.forEach((func, index) => {
        ...
        lines.push(
          `| ${index + 1} | ${func.address} | ${func.name || 'unknown'} | ${func.score.toFixed(2)} | ${tags} |`
        )
      })
  ...
  if (binaryProfile) {
    ...
    if (binaryProfile.export_surface.notable_exports.length > 0) {
      lines.push(
        `- **Notable Exports:** ${binaryProfile.export_surface.notable_exports.join(', ')}`
      )
    }
    ...
    if (binaryProfile.export_dispatch_profile.likely_dispatch_model !== 'none') {
      lines.push(
        `- **Likely Dispatch Model:** ${binaryProfile.export_dispatch_profile.likely_dispatch_model}`
      )
    }
    if (binaryProfile.export_dispatch_profile.registration_exports.length > 0) {
      lines.push(
        `- **Registration Exports:** ${binaryProfile.export_dispatch_profile.registration_exports.join(', ')}`
      )
    }
    ...
    if (binaryProfile.com_profile.class_factory_exports.length > 0) {
      lines.push(
        `- **Class Factory Exports:** ${binaryProfile.com_profile.class_factory_exports.join(', ')}`
      )
    }
    ...
    if (binaryProfile.com_profile.interface_hints.length > 0) {
      lines.push(
        `- **COM Interface Hints:** ${binaryProfile.com_profile.interface_hints.join(', ')}`
      )
    }
    if (binaryProfile.com_profile.registration_strings.length > 0) {
      lines.push(
        `- **COM Registration Strings:** ${binaryProfile.com_profile.registration_strings.join(', ')}`
      )
    }
    if (binaryProfile.host_interaction_profile.callback_exports.length > 0) {
      lines.push(
        `- **Callback Exports:** ${binaryProfile.host_interaction_profile.callback_exports.join(', ')}`
      )
    }
    ...
    if (binaryProfile.host_interaction_profile.callback_strings.length > 0) {
      lines.push(
        `- **Callback Strings:** ${binaryProfile.host_interaction_profile.callback_strings.join(', ')}`
      )
    }
    if (binaryProfile.host_interaction_profile.service_hooks.length > 0) {
      lines.push(
        `- **Service Hooks:** ${binaryProfile.host_interaction_profile.service_hooks.join(', ')}`
      )
    }
    if (binaryProfile.host_interaction_profile.host_hints.length > 0) {
      lines.push(
        `- **Host Hints:** ${binaryProfile.host_interaction_profile.host_hints.join(', ')}`
      )
    }
  ...
  if (staticCapabilities) {
    ...
    lines.push(
      `- **Capability Namespaces:** ${staticCapabilities.behavior_namespaces.join(', ') || 'none'}`
    )
  } else {
  ...
  if (dynamicEvidence) {
    ...
    lines.push(
      `- **Source Formats:** ${(dynamicEvidence.source_formats || []).join(', ') || 'none'}`
    )
  ...
  if (provenance) {
    ...
    lines.push('')
    lines.push(
      `- **Runtime Artifact IDs:** ${provenance.runtime.artifact_ids.join(', ') || 'none'}`
    )
    lines.push(
      `- **Runtime Session Tags:** ${provenance.runtime.session_tags.join(', ') || 'none'}`
    )
    lines.push(
      `- **Runtime Latest Artifact At:** ${provenance.runtime.latest_artifact_at || 'N/A'}`
    )
    lines.push(
      `- **Static Capability Artifact IDs:** ${provenance.static_capabilities.artifact_ids.join(', ') || 'none'}`
    )
    lines.push(
      `- **Static Capability Session Tags:** ${provenance.static_capabilities.session_tags.join(', ') || 'none'}`
    )
    lines.push(
      `- **PE Structure Artifact IDs:** ${provenance.pe_structure.artifact_ids.join(', ') || 'none'}`
    )
    lines.push(
      `- **PE Structure Session Tags:** ${provenance.pe_structure.session_tags.join(', ') || 'none'}`
    )
    lines.push(
      `- **Compiler/Packer Artifact IDs:** ${provenance.compiler_packer.artifact_ids.join(', ') || 'none'}`
    )
    lines.push(
      `- **Compiler/Packer Session Tags:** ${provenance.compiler_packer.session_tags.join(', ') || 'none'}`
    )
    lines.push(
      `- **Semantic Artifact IDs:** ${provenance.semantic_explanations.artifact_ids.join(', ') || 'none'}`
    )
    lines.push(
      `- **Semantic Session Tags:** ${provenance.semantic_explanations.session_tags.join(', ') || 'none'}`
    )
    lines.push(
      `- **Semantic Latest Artifact At:** ${provenance.semantic_explanations.latest_artifact_at || 'N/A'}`
    )
    lines.push('')
  }
  ...
  if (ghidraExecution) {
    ...
    if (ghidraExecution.progress_stages.length > 0) {
      ...
      for (const stage of ghidraExecution.progress_stages) {
        lines.push(
          `  - ${stage.progress}% ${stage.stage}${stage.detail ? ` (${stage.detail})` : ''}`
        )
      }
    }
    lines.push('')
    ...
    try {
      ...
      if (binaryRoleProfileHandler) {
        ...
        if (binaryRoleProfileResult.ok && binaryRoleProfileResult.data) {
          binaryProfile = binaryRoleProfileResult.data as z.infer<
      ...
      if (input.compare_static_scope) {
        const [baselineCapabilities, baselinePeStructure, baselineCompilerPacker] =
          await Promise.all([
            loadStaticAnalysisArtifactSelection<z.infer<typeof StaticCapabilityTriageDataSchema>>(
              workspaceManager,
              database,
      ...
      switch (format) {
        ...
        case 'json':
          const confidenceSemantics = buildReportConfidenceSemantics({
            score: dynamicEvidence?.executed ? 0.72 : dynamicEvidence ? 0.58 : 0.42,
            evidenceScope: input.evidence_scope,
            runtimeLayers: dynamicEvidence?.confidence_layers?.map((item) => item.layer) || [
              'static_only',
            ],
            executedTracePresent: dynamicEvidence?.executed || false,
          })
