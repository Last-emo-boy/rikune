...
function Invoke-RuntimePrivateEnvSnapshotOperation {
    ...
    try {
        ...
        if ($nativeExitCode -ne 0) {
            ...
            if (
                $nativeFailureText -match '(?im)^\s*(?:node(?:\.exe)?\s*:\s*)?Error:\s+RIKUNE_PRIVATE_ENV_FAILURE=(acl-(?:snapshot-match|pre-unlink)-child-other):\s+Unable to verify Windows ACL \(child-status=(0xFFFF0000); child-stderr=(silent|ascii-like|ascii-nul-le-shape|opaque|legacy-en-clr-load-shape|legacy-en-clr-start-shape|legacy-en-appdomain-shape|legacy-en-managed-entry-load-shape|legacy-en-managed-entry-dispatch-shape|legacy-en-managed-entry-invoke-shape|legacy-en-managed-exception-shape|legacy-en-initial-session-state-type-init-shape|legacy-en-psetwlog-type-init-shape|legacy-en-etw-activity-type-init-shape|legacy-en-consolehost-type-init-shape|legacy-en-task-type-init-shape|legacy-en-utils-type-init-shape|legacy-en-psversioninfo-type-init-shape|legacy-en-pssnapin-reader-type-init-shape|legacy-en-sma-namespace-type-init-shape|legacy-en-microsoft-powershell-namespace-type-init-shape|legacy-en-system-namespace-type-init-shape|legacy-en-bounded-ascii-type-init-shape|legacy-en-consolehost-startup-shape)\)[ \t]*\r?$'
            ) {
