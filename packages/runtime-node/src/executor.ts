...
function resolveRuntimeContract(task: ExecuteTask): ToolRuntimeContract | undefined {
  if (task.runtime) {
    return task.runtime
  }
...
export async function executeTask(
  ...
  const spec = resolveRuntimeContract(task)
  if (!spec) {
    return {
      ok: false,
      taskId: task.taskId,
      errors: [
        `Unknown tool: ${task.tool}. No runtime contract provided and no fallback heuristic matched.`,
      ],
      logs,
    }
  }
...
export function isRuntimeContractSupported(contract: ToolRuntimeContract): boolean {
  return getRuntimeBackendCapability(contract) !== undefined
}
...
function findCdbPath(): string | null {
  const toolCacheMatch = findRuntimeToolById('cdb')
  if (toolCacheMatch) {
...
function findProcDumpPath(): string | null {
  const toolCacheMatch = findRuntimeToolById('procdump')
  if (toolCacheMatch) {
    return toolCacheMatch.path
  }
  return null
...
function findProcMonPath(): string | null {
  const toolCacheMatch = findRuntimeToolById('procmon')
  return toolCacheMatch?.path ?? null
}

function findSysmonPath(): string | null {
  const toolCacheMatch = findRuntimeToolById('sysmon')
  return toolCacheMatch?.path ?? null
}
