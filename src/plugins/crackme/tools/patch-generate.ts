...
export const PatchGenerateInputSchema = z.object({
  ...
  patches: z
    .array(
      z.object({
        ...
        size: z
          ...
          .max(32)
          .optional()
          .describe('Number of bytes to NOP (for nop type)'),
    ...
    .min(1)
    .describe('List of patches to apply'),
...
function vaToFileOffset(
  va: number,
  imageBase: number,
  sections: Array<{ va: number; rawOffset: number; rawSize: number; virtualSize: number }>
): number {
...
function parsePESections(data: Buffer): {
  ...
  const imageBase = is64
    ? Number(data.readBigUInt64LE(peOffset + 24 + 24))
    : data.readUInt32LE(peOffset + 24 + 28)
  ...
  return async (args: z.infer<typeof PatchGenerateInputSchema>): Promise<WorkerResult> => {
    ...
    try {
      ...
      if (!policyDecision.allowed) {
        return {
          ok: false,
          errors: [policyDecision.reason || 'Patch generation denied by policy guard.'],
        ...
        switch (patch.type) {
          ...
          case 'jmp_always': {
            ...
            if (COND_JUMPS_SHORT.has(originalByte)) {
              ...
              data[fileOffset] === COND_JUMPS_NEAR_PREFIX &&
              ...
              warnings.push(
                `${patch.address}: not a recognized conditional jump (byte: 0x${originalByte.toString(16)}), applying JMP SHORT`
              )
              patchBytes = Buffer.from([JMP_SHORT, data[fileOffset + 1]])
            }
            break
      ...
      if (args.output_format === 'ips' || args.output_format === 'both') {
        ...
        try {
          const artRef = await persistStaticAnalysisJsonArtifact(
            ...
            'ips_patch',
            'patch-ips',
            { ips_path: ipsPath, patch_count: appliedPatches.length }
          )
