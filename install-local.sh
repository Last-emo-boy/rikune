...
# Requires: Node.js 22+, CPython 3.12 x86_64, bash
# =============================================================================
set -euo pipefail
...
unset RIKUNE_API_KEY RIKUNE_ANALYZER_API_KEY RIKUNE_STAGE_LOCAL_ENV_PATH \
  ...
  STAGED_LOCAL_ENV_BASE64 RUNTIME_HOST_AGENT_API_KEY HOST_AGENT_API_KEY \
...
Options:
  ...
  -r MODE  Runtime mode: disabled, auto-sandbox, manual, remote-sandbox (default: $RUNTIME_MODE)
  -h       Show this help
EOF
  exit 1
}
...
case "$RUNTIME_MODE" in
  ...
  auto-sandbox)
    err "RUNTIME_MODE=auto-sandbox requires a Windows-native analyzer. Use install-local.ps1 on Windows."
    exit 1
    ;;
  *)
...
STAGED_LOCAL_ENV_BASE64="$(RIKUNE_STAGE_LOCAL_ENV_PATH="$ENV_FILE" node "$LOCAL_ENV_WRITER")"
export -n STAGED_LOCAL_ENV_BASE64 2>/dev/null || true
...
RIKUNE_LOCAL_EXISTING_ENV_BASE64="$STAGED_LOCAL_ENV_BASE64" \
