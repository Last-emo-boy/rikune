...
export const IlXrefsInputSchema = z.object({
  ...
  token: z
    .string()
    .describe('Metadata token in hex (e.g. 0x04000012) or fully-qualified member name'),
  token_type: z.enum(['field', 'method', 'type']).describe('Kind of token being queried'),
  include_generic_instantiations: z
    .boolean()
    .default(true)
    .describe('Resolve generic instantiation contexts'),
  max_results: z
    .number()
    .min(1)
    .max(5000)
