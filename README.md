...
- Plugin architecture with 110 built-in plugins and external plugin discovery.
...
Rikune currently includes 110 built-in plugins under `src/plugins/<id>/`. Plugins can register tools, declare dependencies, expose configuration schema, participate in lifecycle hooks, provide Docker metadata, and declare bounded Worker-backed tools through `workerBackend` metadata.
