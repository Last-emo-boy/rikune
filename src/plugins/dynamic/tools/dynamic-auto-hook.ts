...
const CAPABILITY_HOOKS: Record<string, Array<{ api: string; module: string; reason: string }>> = {
  ...
  anti_debugging: [
    ...
    { api: 'CheckRemoteDebuggerPresent', module: 'kernel32', reason: 'Remote debugger check' },
    {
      ...
      module: 'ntdll',
      reason: 'Process info query (anti-debug)',
    },
...
export const DynamicAutoHookInputSchema = z.object({
  ...
  capabilities: z
    .array(z.string())
    .optional()
    .describe('Override detected capabilities for hook generation'),
...
function generateFridaScript(
  hooks: Array<{ api: string; module: string; reason: string }>
): string {
  const lines: string[] = [
  ...
  for (const hook of hooks) {
    ...
    lines.push(
      `        send({ type: 'api_return', api: '${hook.api}', retval: retval.toString() });`
    )
    lines.push(`      }`)
    lines.push(`    });`)
    lines.push(`  }`)
...
export function createDynamicAutoHookHandler(deps: PluginToolDeps) {
  ...
  return async (args: z.infer<typeof DynamicAutoHookInputSchema>): Promise<WorkerResult> => {
    ...
    try {
      ...
      if (!policyDecision.allowed) {
        return {
          ok: false,
          errors: [policyDecision.reason || 'Auto-hook generation denied by policy guard.'],
      ...
      if (capabilities.length === 0) {
        ...
        if (Array.isArray(evidence)) {
          for (const entry of evidence) {
            if (
              entry.evidence_family === 'capability_triage' ||
              entry.evidence_family === 'static_triage'
            ) {
              try {
                const data =
                ...
                const caps =
                  ...
                  data?.data?.capability_categories ??
                  []
        ...
        if (capabilities.length === 0) {
          warnings.push(
            'No capability triage results found. Run static.capability.triage first for targeted hooks.'
          )
