...
export const SafeRunInputSchema = z.object({
  ...
  entry_class: z
    .string()
    .optional()
    .describe('Fully-qualified class name to invoke (auto-detected if omitted)'),
  entry_method: z
    .string()
    .optional()
    .describe('Method name to invoke (defaults to Main / EntryPoint)'),
  ...
  enable_network_sinkhole: z
    .boolean()
    .default(true)
    .describe('Redirect all network calls to local sinkhole'),
  sinkhole_response_body: z
    .string()
    .optional()
    .describe('Custom HTTP response body returned by the sinkhole'),
  hook_assembly_load: z
    .boolean()
    .default(true)
    .describe('Hook Assembly.Load(byte[]) and log payloads'),
  hook_resource_resolve: z
    .boolean()
    .default(true)
    .describe('Hook AppDomain.ResourceResolve events'),
  hook_create_decryptor: z
    .boolean()
    .default(true)
    .describe('Hook SymmetricAlgorithm.CreateDecryptor calls'),
  hook_method_invoke: z.boolean().default(true).describe('Hook MethodInfo.Invoke calls'),
  dump_loaded_assemblies: z
    .boolean()
    .default(true)
    .describe('Dump dynamically loaded assemblies to workspace'),
})
...
export function createSafeRunHandler(deps: PluginToolDeps) {
  ...
  return async (args: z.infer<typeof SafeRunInputSchema>): Promise<WorkerResult> => {
    const t0 = Date.now()
    // Backend gate
    const pythonBackend = resolveExecutable({
      pathCandidates: [pythonCmd, 'python3', 'python'],
